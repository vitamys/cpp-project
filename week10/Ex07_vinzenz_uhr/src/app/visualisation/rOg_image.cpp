...
void rOg_image::resizeEvent(QResizeEvent *event)
{
    ...
    //  the previous area in the scene
    this->fitInView(QRect(P1.toPoint(),P2.toPoint()),Qt::KeepAspectRatio);
}

// Overloaded function that normalizes the image
void rOg_image::mouseDoubleClickEvent(QMouseEvent *event)
{
    event->accept();
    fitImage();
}

void rOg_image::dropEvent(QDropEvent *event)
{
    // propagate to parent widget
    event->ignore();
}

void rOg_image::dragEnterEvent(QDragEnterEvent *event)
{
    // propagate to parent widget
    event->ignore();
}

void rOg_image::dragMoveEvent(QDragMoveEvent *event)
{
    // propagate to parent widget
    event->ignore();
}

void rOg_image::saveImage()
{
